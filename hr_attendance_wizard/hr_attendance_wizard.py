...
class hr_attendance_add_wizard(models.TransientModel):
    ...
    def get_values_dict(self, type):
        """Override to add more values to attendance creation."""
        if type == 'sign_in':
            return {
    ...
    def create_attendances(self):
        ...
        values = self.get_values_dict('sign_in')
        self.env.cr.execute("INSERT INTO hr_attendance (%s) VALUES (%s)" % (','.join(values.keys()), ','.join(['%s' for v in values.keys()])), [values[v] for v in values.keys()])
        values = self.get_values_dict('sign_out')
        self.env.cr.execute("INSERT INTO hr_attendance (%s) VALUES (%s)" % (','.join(values.keys()), ','.join(['%s' for v in values.keys()])), [values[v] for v in values.keys()])
